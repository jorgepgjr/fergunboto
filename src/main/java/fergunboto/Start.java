...
public class Start {
	...
	public static void matchImage() throws Exception {
		...
		Mat img = Highgui.imread("src/main/resources/mesa.tif", CvType.CV_32FC1);
		Mat templ = Highgui.imread("src/main/resources/pot_template.tif");
...
//		Core.rectangle(img, matchLoc, pontoFinal, new Scalar(0, 255, 0));
		...
		Point pontoFinalDoValor = new Point(pontoFinal.x + 20, pontoFinal.y - 25);
		Point pontoInicialDoValor = new Point(pontoFinal.x, pontoFinal.y - 6);
		
		// Rect do valor do pot
		Rect valor = new Rect(pontoInicialDoValor, pontoFinalDoValor);
//		Core.rectangle(img, pontoFinal, pontoFinalDoValor, new Scalar(255, 0, 0));
		...
		Highgui.imwrite("src/main/resources/saida.jpg", img);
		 ...
		 Highgui.imwrite("src/main/resources/valorpot.bmp", x);

		File imagemValorPot = new File("src/main/resources/valorpot.bmp");
		...
		try {
			...
			Process pr = rt
					.exec("tesseract src/main/resources/valorpot.bmp src/main/resources/valorpot contrast negate digits");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
